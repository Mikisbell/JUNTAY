...
# Lista completa de distritos (ejemplo - pegar aquí tu lista completa)
distritos_lista = """
...
-- =====================================================
-- Script para insertar los 1,812 distritos oficiales del Perú
-- Fuente: Instituto Nacional de Estadística e Informática (INEI)

"""
    
    for i, linea in enumerate(distritos_lista.split('\n'), 1):
        if '|' in linea:
            distrito, provincia, departamento = [x.strip() for x in linea.split('|')]
            codigo = f"{i:06d}"
            
            sql += f"""-- {distrito}
INSERT INTO distritos (provincia_id, departamento_id, codigo, nombre, nombre_completo, ubigeo_inei, activo) 
SELECT p.id, d.id, '{codigo}', '{distrito}', 'Distrito de {distrito}', '{codigo}', true
...
sql_completo = generar_sql()
with open('003_insert_distritos_COMPLETOS_GENERADO.sql', 'w', encoding='utf-8') as f:
    f.write(sql_completo)

print("✅ Archivo SQL generado: 003_insert_distritos_COMPLETOS_GENERADO.sql")
print("✅ Listo para ejecutar en Supabase")
