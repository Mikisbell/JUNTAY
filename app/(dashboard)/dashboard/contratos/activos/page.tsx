...
export default async function ContratosActivosPage() {
  const creditos = await getCreditos()
  const activos = creditos.filter(c => c.estado === "vigente")

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Contratos activos</h1>
          <p className="text-sm text-gray-600">Todos los contratos vigentes con saldo pendiente.</p>
        </div>
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Listado de contratos activos</CardTitle>
        </CardHeader>
        <CardContent>
          {activos.length === 0 ? (
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    ...
                    <th className="text-left p-3 text-[11px] font-medium text-gray-500 tracking-wide uppercase">Monto</th>
                ...
                <tbody>
                  {activos.map((c) => (
                    <tr key={c.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3">
                        <p className="font-medium text-gray-900">S/ {c.monto_prestado.toFixed(2)}</p>
                        <p className="text-[11px] text-gray-500">Total: S/ {c.monto_total.toFixed(2)}</p>
                      </td>
                      <td className="p-3">
                        <p className="font-medium text-gray-900">S/ {c.saldo_pendiente.toFixed(2)}</p>
                      </td>
                      <td className="p-3">
                        <Badge variant="default" className="text-[11px]">vigente</Badge>
                      </td>
